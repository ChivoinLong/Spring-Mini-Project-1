...
(function ($, window, document, undefined) {
    ...
    var defaults = {
        ...
        // true – accept all
        // false – deny all
        // ['sortable-2'] – accept from instances matching #sortable-2
        select2: {
        ...
        paramsDataKey: '__sortable_params' // The property under which internal settings will be serialized
    };
    ...
    Plugin.prototype = {
        ...
        clickStartEditEventHandler: function (event) {
            ...
            var igniteKeypressEnterEndEditEventHandler = function (el) {
                el.each(function (c, item) {
                    ...
                    if (item.data('sortable_keypressEnterEndEditEventHandler')) return;

                    item.data('sortable_keypressEnterEndEditEventHandler', true);
            ...
            if (item.data('sortable_clickEndEditEventHandler') !== true) {
                _this.clickEndEditEventHandler(item);
                item.data('sortable_clickEndEditEventHandler', true);
            }
        ...
        serialize: function () {
            ...
            step = function (level, depth) {
                ...
                items.each(function () {
                    ...
                    // Filter out sortable_ prefixed data values
                    $.each(li.data(), function (key, i) {

                        // Skip when the prefix is present
                        if (key.indexOf('sortable_') === 0) return;
        ...
        dragStop: function (e) {
            ...
            if ($(el).parents(this.options.rootClass).data('sortable-id') !== $(this.$instance).data('sortable-id')) {
                $(el).parents(this.options.rootClass.dot()).sortable()._plugin.options.event.onItemDrop.forEach(function (cb, i) {
                    cb(el, e);
                });
            }
        },
        ...
        dragMove: function (e) {
            ...
            hasRootChanged = this.dragRootEl.data('sortable-id') !== $rootListOfPointingElement.data('sortable-id');
            ...
            if (this.options.allowListMerging !== true) {
                ...
                else if (hasRootChanged && $rootListOfPointingElement.data('sortable') && $rootListOfPointingElement.data('sortable').options.allowListMerging.indexOf(this.$instance.attr('id')) === -1) return;
            }
            /**
             * @dev-since 0.48.59
     ...
     */
    $.fn.sortable = function (params) {
        var lists = this.first(),
            sortable = $(this),
            plugin = sortable.data("sortable") || new Plugin(this, params),
            pPlugin = new PublicPlugin(plugin, lists);

        if (params) plugin.options = $.extend(true, {}, defaults, params);
        if (!sortable.data("sortable") || params) sortable.data("sortable", plugin);
        if (!sortable.data("sortable-id")) {
            var pseudoRandomNumericKey = Math.random().toString().replace(/\D/g, '');
            sortable.data('sortable-id', pseudoRandomNumericKey);
        }
